### 20221128
- lua
  - 一旦运行lua脚本，目前的单线程模型会阻塞所有客户端连接
- bitmap
  - 典型应用场景：去重/统计
    - 如，qq号码去重. 显然，数据是一个个输入的，可以将输入的当前的序号当成位图的位数据，比如，第一个输入的数据当成第一位，以此类推。40亿/1024/1024=477MB，输入后？？？？？
- hyperloglog
  - 基于概率论，举个例子，抛硬币，正反两面，假设以当抛到证明的inxex作为k，且将这次抛掷过程成为伯克利过程。假设抛掷n次，将其中k的最大值记为Kmax。 经过概率推算，当n约为2^kmax时候成立。
  - 当统计如独立用户uv登陆等不需要太精确的数据时候，可以使用该类型。本质上就是利用概率推算n的大小。
### 20221127
- lua 
  - 分布式锁
